...
public class G {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < SuffixAutomaton.maxn; i++) {
			SuffixAutomaton.to[i] = new HashMap<Character, Integer>();
		}

		SuffixAutomaton.addCharacter(city);
		...
		for (int cas = 0; cas < N; cas++) {
			...
			for (char act : friend.toCharArray()) {
				if (actNode.containsKey(act)) {
					actNode = SuffixAutomaton.to[actNode.get(act)];
				} else {
					count++;
					actNode = SuffixAutomaton.to[0];
					if (actNode.containsKey(act)) {
						actNode = SuffixAutomaton.to[actNode.get(act)];
					} else {
						count = -1;
